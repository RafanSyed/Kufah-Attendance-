...
import TeacherModel from "./teacher/models";
...
export const validateTeacher = async (id: number): Promise<boolean> => {
...
export const validateEmail = async (email: string): Promise<boolean> => {
  ...
  const teacherExists = await TeacherModel.findOne({ where: { email } });
  if (teacherExists) {
    throw new Error(`Email already exists for a teacher: ${email}`);
  }
